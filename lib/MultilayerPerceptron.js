...
var NeuronLayer = function NeuronLayer(neuronCount) {
  _classCallCheck(this, NeuronLayer);

  this._neuronCount = neuronCount;
};

var InputLayer = function (_NeuronLayer) {
  _inherits(InputLayer, _NeuronLayer);

  function InputLayer(neuronCount) {
    _classCallCheck(this, InputLayer);

    return _possibleConstructorReturn(this, (InputLayer.__proto__ || Object.getPrototypeOf(InputLayer)).call(this, neuronCount));
  }

  _createClass(InputLayer, [{
    key: 'fire',
    value: function fire(inputValues) {
    ...
    key: 'getDendrites',
    value: function getDendrites() {
      var neurons = this.getNeurons();
      var allDendrites = [];

      for (var i = 0; i < neurons.length; i++) {
        var neuron = neurons[i];
        var dendrites = neuron.getInboundDendrites();
...
var MultilayerPerceptron = function () {
  function MultilayerPerceptron(neuronCounts) {
